...
function call(){
    ...
    time LD_LIBRARY_PATH=$DEBUG_BINARY_PATH:$OLD_LD_PATH java -agentlib:$2=1 -cp java_classes:"$CLASSPATH" calc.A

    echo call with agent, release
    time LD_LIBRARY_PATH=$RELEASE_BINARY_PATH:$OLD_LD_PATH java -agentlib:$2=1 -cp java_classes:"$CLASSPATH" calc.A
}
